<h3><?php _e('Products attributes', 'products_attributes') ; ?></h3>
<div>
    <table>
        <tr>
            <td>
                <label for="make"><?php _e('Make', 'products_attributes'); ?></label>
                <input type="text" id="make" name="make" style="border:0; color:#f6931f; font-weight:bold;" />
            </td>
        </tr>
        <tr>
            <td>
                <label for="model"><?php _e('Model', 'products_attributes'); ?></label>
                <input type="text" id="model" name="model" style="border:0; color:#f6931f; font-weight:bold;" />
            </td>
        </tr>
    </table>
</div>