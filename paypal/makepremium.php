                    ...
                    <div style="float:left; width: 50%;">
                        ...
                        <?php if(osc_is_web_user_logged_in()) {
                                ...
                                if(isset($wallet['f_amount']) && $wallet['f_amount']>=$category_fee) {
                                    wallet_button($category_fee, sprintf(__("Premium fee for item %d at %s", "paypal"), $item['pk_i_id'], osc_page_title()), $item['fk_i_user_id']."|".$item['pk_i_id']."|".$item['s_contact_email'], "201x".$item['fk_i_category_id']."x".$item['pk_i_id']);
                                } else {
                                    paypal_button($category_fee, sprintf(__("Premium fee for item %d at %s", "paypal"), $item['pk_i_id'], osc_page_title()), $item['fk_i_user_id']."|".$item['pk_i_id']."|".$item['s_contact_email'], "201x".$item['fk_i_category_id']."x".$item['pk_i_id']);
                                }
                            } else {
                                paypal_button($category_fee, sprintf(__("Premium fee for item %d at %s", "paypal"), $item['pk_i_id'], osc_page_title()), $item['fk_i_user_id']."|".$item['pk_i_id']."|".$item['s_contact_email'], "201x".$item['fk_i_category_id']."x".$item['pk_i_id']);
                            }; ?>
